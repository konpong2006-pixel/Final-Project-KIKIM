...
exports.DEFAULT_ADAPTIVE_PREFERENCES = void 0;
exports.parseClockMinutes = parseClockMinutes;
...
exports.nextBangkokDayStart = nextBangkokDayStart;
const MINUTE_MS = 60_000;
exports.DEFAULT_ADAPTIVE_PREFERENCES = {
...
function validateMovableScheduleItem(item) {
    if (!item.isFlexible)
        return { code: "fixed", ok: false };
    if (item.isLocked)
        return { code: "locked", ok: false };
    if (!item.allowAiReschedule)
        return { code: "disabled", ok: false };
    if (item.googleEventId || item.source === "google-calendar")
        return { code: "external", ok: false };
    return { ok: true };
}
function adaptiveTimePeriod(hour) {
    if (hour >= 5 && hour < 8)
...
function zonedDayStart(referenceMs, timeZone, dayOffset = 0) {
...
function overlaps(startMs, endMs, item, bufferMinutes) {
...
function unavailable(startMs, endMs, preferences, category, options = {}) {
    ...
    if (!withinClockWindow(start.minuteOfDay, end.minuteOfDay + 1, earliest, latest))
        return true;
    ...
    if (wake !== null && sleep !== null && !withinClockWindow(start.minuteOfDay, end.minuteOfDay + 1, wake, sleep))
        return true;
    return preferences.unavailablePeriods.some((period) => {
        if (period.category && period.category !== category)
            return false;
        if (!period.days.includes(start.dayOfWeek))
            return false;
...
function dayWorkloadMinutes(startMs, items, timeZone) {
    const dateKey = zonedParts(startMs, timeZone).dateKey;
    return items.reduce((total, item) => {
        if (zonedParts(item.startMs, timeZone).dateKey !== dateKey)
            return total;
        return total + Math.max(0, (item.endMs - item.startMs) / MINUTE_MS);
    }, 0);
}
function matchingPattern(patterns, category, startMs, timeZone) {
...
function scoreSlot(request, startMs, endMs) {
...
function validateCandidateSlot(request, startMs, endMs, options = {}) {
    ...
    if (request.deadlineMs && endMs > request.deadlineMs) {
        return { code: "deadline", message: "ช่วงเวลานี้เลยกำหนดส่ง", ok: false };
    }
    if (startMs < request.earliestStartMs || endMs > request.latestEndMs) {
        return { code: "outside_availability", message: "ช่วงเวลานี้อยู่นอกช่วงที่กำลังตรวจสอบ", ok: false };
    }
...
function findAdaptiveTimeSlots(request, maximumResults = 8) {
    if (request.durationMinutes > request.preferences.maximumFocusSessionMinutes && request.category !== "rest")
        return [];
...
function confidenceFor(observationCount, completionRate, thresholds) {
    if (observationCount < thresholds.lowObservationCount)
        return { level: "insufficient", score: 0 };
    if (observationCount < thresholds.mediumObservationCount)
        return { level: "low", score: 0.3 };
    if (observationCount < thresholds.highObservationCount)
        return { level: "medium", score: 0.6 };
    return { level: "high", score: Math.min(0.95, 0.75 + Math.abs(completionRate - 0.5) * 0.4) };
}
function calculateSchedulingPatterns(observations, thresholds = exports.DEFAULT_ADAPTIVE_PREFERENCES.thresholds, timeZone = exports.DEFAULT_ADAPTIVE_PREFERENCES.timeZone) {
    ...
    return [...grouped.entries()].map(([key, items]) => {
        const [category, dayText] = key.split(":");
        ...
        const starts = items.map((item) => item.actualStartMs ?? item.updatedStartMs ?? item.originalStartMs).filter((value) => value !== null);
        ...
        const durations = items.map((item) => item.actualDurationMinutes).filter((value) => value !== null && value > 0);
...
function nextBangkokDayStart(nowMs) {
