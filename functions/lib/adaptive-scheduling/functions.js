...
function requiredUid(request) {
    ...
    if (!uid)
        throw new https_1.HttpsError("unauthenticated", "Please sign in before using adaptive scheduling.");
    return uid;
}
...
function validClock(value, fallback) {
    ...
    return (0, engine_1.parseClockMinutes)(candidate) === null ? fallback : candidate;
}
function category(value) {
    const candidate = text(value, 40).toLowerCase().replace(/[ -]+/g, "_");
    if (types_1.ADAPTIVE_ACTIVITY_CATEGORIES.includes(candidate))
        return candidate;
    if (/gaming|game|เล่นเกม|เกม|ไฟต์/i.test(candidate))
        return "gaming";
    if (/program|code|coding|dev|เขียนโปรแกรม/i.test(candidate))
        return "programming";
    if (/exercise|workout|gym|วิ่ง|ออกกำลัง/i.test(candidate))
        return "exercise";
    if (/read|หนังสือ|อ่าน/i.test(candidate))
        return "reading";
    if (/study|เรียน|ทบทวน/i.test(candidate))
        return "study";
    if (/assign|homework|งานส่ง|การบ้าน/i.test(candidate))
        return "assignment";
    return "other";
}
...
function parsedLocalClock(hourValue, minuteValue, prefixValue, suffixValue) {
    ...
    if (!Number.isInteger(hour) || !Number.isInteger(minute) || minute < 0 || minute > 59)
        return null;
    if (/ทุ่ม/.test(marker))
        hour = hour === 6 ? 0 : hour + 18;
    else if (/(?:pm|โมงเย็น|บ่าย|เย็น|ค่ำ|กลางคืน|ดึก)/.test(marker) && hour < 12)
        hour += 12;
    else if (/(?:am|ตี|เช้า)/.test(marker) && hour === 12)
        hour = 0;
    if (hour < 0 || hour > 23)
        return null;
    return `${String(hour).padStart(2, "0")}:${String(minute).padStart(2, "0")}`;
}
function relationClock(message, relation) {
...
function isExclusiveAfterClock(message) {
    const normalized = normalizeThaiDigits(message).toLowerCase();
...
function namedClockSemantics(message) {
    ...
    if (new RegExp(`(?:ตั้งแต่|ไม่ก่อน|from)\\s*(?:เวลา)?\\s*${phrase}`, "i").test(normalized)) {
        return { clock: isMidnight ? "00:00" : "12:00", exclusive: false, period: isMidnight ? "night" : "noon", relation: "after" };
    }
    if (new RegExp(`(?:ก่อน|ไม่เกิน|ไม่หลัง|before|by)\\s*(?:เวลา)?\\s*${phrase}`, "i").test(normalized)) {
        return { clock: isMidnight ? "00:00" : "12:00", exclusive: false, period: isMidnight ? "night" : "noon", relation: "before" };
    }
    return { clock: isMidnight ? "00:00" : "12:00", exclusive: false, period: isMidnight ? "night" : "noon", relation: "exact" };
}
...
function fallbackAdaptiveNaturalLanguageIntent(message, temporalContext) {
    ...
    const preferenceMode = /ไม่|อย่า|ห้าม|avoid|never/i.test(message) ? "avoid" : intent === "set_preference" ? "prefer" : null;
...
function requestedWindowForIntent(intent, durationMinutes) {
    const hasExplicitClock = Boolean(intent.earliestLocalStartTime || intent.latestLocalStartTime);
    const period = !hasExplicitClock && intent.preferredPeriod ? REQUESTED_PERIOD_WINDOWS[intent.preferredPeriod] : null;
    const periodStart = period ? (0, engine_1.parseClockMinutes)(period.startTime) : 0;
    const periodEnd = period ? (0, engine_1.parseClockMinutes)(period.endTime) : 23 * 60 + 59;
    const parsedExplicitEarliest = intent.earliestLocalStartTime ? (0, engine_1.parseClockMinutes)(intent.earliestLocalStartTime) : null;
    const explicitEarliest = parsedExplicitEarliest === null ? null : parsedExplicitEarliest + (intent.earliestLocalStartExclusive ? 1 : 0);
    const explicitLatest = intent.latestLocalStartTime ? (0, engine_1.parseClockMinutes)(intent.latestLocalStartTime) : null;
    if (!period && explicitEarliest === null && explicitLatest === null)
        return undefined;
...
function priority(value) {
    ...
    if (/urgent|ด่วน/.test(candidate))
        return "urgent";
    if (/high|สูง|สำคัญ/.test(candidate))
        return "high";
    if (/low|ต่ำ/.test(candidate))
        return "low";
    return "medium";
}
function plainMetadata(value) {
    ...
    const entries = Object.entries(value).slice(0, 20).flatMap(([key, item]) => {
        ...
        if (!safeKey || !["boolean", "number", "string"].includes(typeof item))
            return [];
        return [[safeKey, typeof item === "string" ? item.slice(0, 200) : item]];
    });
    return Object.fromEntries(entries);
}
function sanitizePreferences(value, base = engine_1.DEFAULT_ADAPTIVE_PREFERENCES) {
    const data = value && typeof value === "object" && !Array.isArray(value) ? value : {};
    const preferredInput = data.preferredTimeByCategory && typeof data.preferredTimeByCategory === "object" ?
        data.preferredTimeByCategory : {};
    const preferredTimeByCategory = { ...base.preferredTimeByCategory };
    Object.entries(preferredInput).slice(0, 10).forEach(([key, period]) => {
        if (!types_1.ADAPTIVE_ACTIVITY_CATEGORIES.includes(key) || !period || typeof period !== "object")
            return;
        const record = period;
        const startTime = validClock(record.startTime, null);
        const endTime = validClock(record.endTime, null);
        if (startTime && endTime)
            preferredTimeByCategory[key] = { endTime, startTime };
    });
    const unavailablePeriods = Array.isArray(data.unavailablePeriods) ? data.unavailablePeriods.slice(0, 30).flatMap((item) => {
        ...
        const itemCategory = types_1.ADAPTIVE_ACTIVITY_CATEGORIES.includes(text(record.category, 40)) ? category(record.category) : undefined;
        return startTime && endTime && days.length ? [{ ...(itemCategory ? { category: itemCategory } : {}), days, endTime, startTime }] : [];
    }) : base.unavailablePeriods;
    const scoreInput = data.scoreWeights && typeof data.scoreWeights === "object" ? data.scoreWeights : {};
    const thresholdInput = data.thresholds && typeof data.thresholds === "object" ? data.thresholds : {};
...
function activityFromDocument(document) {
...
function movable(activity) {
    const result = (0, engine_1.validateMovableScheduleItem)(activity);
    if (result.ok)
        return null;
    const messages = {
    ...
    return { code: "failed-precondition", message: messages[result.code] };
}
...
function verifiedTemporalContext(setting, scheduleItems, requestedScheduleMs) {
    const nowMs = Date.now();
    const today = localDateKey(nowMs, setting.timeZone);
    const hour = localHour(nowMs, setting.timeZone);
    const currentTimePeriod = (0, engine_1.adaptiveTimePeriod)(hour);
    const currentMinute = hour * 60 + Number(localTime(nowMs, setting.timeZone).slice(3, 5));
    const latestMinute = Math.min((0, engine_1.parseClockMinutes)(setting.latestSchedulingTime) ?? 22 * 60, (0, engine_1.parseClockMinutes)(setting.sleepTime ?? "") ?? 24 * 60);
    ...
    const remainingIntervals = scheduleItems.flatMap((item) => {
        ...
        if (!startsToday && !endsToday && !activeNow)
            return [];
    ...
    remainingIntervals.forEach(([startMinute, endMinute]) => {
        ...
        if (!previous || startMinute > previous[1])
            mergedIntervals.push([startMinute, endMinute]);
        else
            previous[1] = Math.max(previous[1], endMinute);
    });
    ...
    const workloadIntervals = nextSevenDays.map((item) => [
    ...
    workloadIntervals.forEach(([startMs, endMs]) => {
        ...
        if (!previous || startMs > previous[1])
            mergedWorkload.push([startMs, endMs]);
        else
...
function createAdaptiveSchedulingFunctions({ db, geminiApiKey, region }) {
    ...
    const settingsRef = (uid) => userRef(uid).collection("settings").doc("adaptiveScheduling");
    ...
    async function writePreferences(uid, patch) {
        ...
        await reference.set({
            ...next,
            createdAt: current.exists ? current.data()?.createdAt ?? firestore_1.FieldValue.serverTimestamp() : firestore_1.FieldValue.serverTimestamp(),
            ownerId: uid,
        ...
        if (validityChanged) {
            ...
            if (!pending.empty) {
                ...
                pending.docs.forEach((document) => batch.update(document.ref, {
                    ...
                    status: "expired",
                    updatedAt: firestore_1.FieldValue.serverTimestamp(),
                    validUntil: firestore_1.Timestamp.now(),
                }));
                await batch.commit();
            }
        }
    ...
    async function sendAdaptiveNotification(uid, title, body, data) {
        ...
        try {
            ...
            if (invalid.length) {
                const batch = db.batch();
                invalid.forEach((reference) => batch.update(reference, { active: false, updatedAt: firestore_1.FieldValue.serverTimestamp() }));
                await batch.commit();
            }
        }
        catch (error) {
    ...
    async function sendAdaptiveNotificationBestEffort(uid, title, body, data) {
        try {
    ...
    async function listPatterns(uid) {
    ...
    async function constraints(uid, fromMs, toMs, excludeActivityId) {
        const from = firestore_1.Timestamp.fromMillis(fromMs - DAY_MS);
        const to = firestore_1.Timestamp.fromMillis(toMs + DAY_MS);
        ...
        const scheduleItems = scheduleSnapshot.docs.flatMap((document) => {
            const data = document.data();
        ...
        suggestionSnapshot.docs.forEach((document) => {
            const data = document.data();
            if (text(data.scheduleItemId, 128) === excludeActivityId)
                return;
            ...
            if (startMs === null || endMs === null || endMs <= fromMs || startMs >= toMs || expiresAt <= Date.now())
                return;
            scheduleItems.push({
    ...
    function deterministicExplanation(activity, pattern, startMs, benefit, timeZone) {
    ...
    async function createSuggestion(uid, activityId, automatic = false, requestedStartMs, enrichWithGemini = true, requiredLocalTimeWindow, requiredLocalDate) {
        ...
        if (!snapshot.exists)
            throw new https_1.HttpsError("not-found", "ไม่พบงานหรือกิจกรรมที่ต้องการจัดเวลา");
        const activity = activityFromDocument({ id: snapshot.id, data: () => snapshot.data() ?? {} });
        const blocked = movable(activity);
        if (blocked)
            throw new https_1.HttpsError(blocked.code, blocked.message);
        ...
        if (existing) {
            ...
            const stillValid = startMs >= nowMs + SUGGESTION_MINIMUM_LEAD_MS &&
                endMs > startMs &&
                (0, engine_1.validateCandidateSlot)(request, startMs, endMs).ok;
            if (stillValid) {
                return {
                    ...
                    createdAt: data.createdAt?.toDate?.().toISOString() ?? new Date(nowMs).toISOString(),
                    ...
                    id: existing.id,
                    mode: data.mode === "automatic" ? "automatic" : "suggestion",
            ...
            await existing.ref.update({
                ...
                status: "expired",
                updatedAt: firestore_1.FieldValue.serverTimestamp(),
                validUntil: firestore_1.Timestamp.fromMillis(nowMs),
            });
        }
        const rejectedStarts = new Set(sameActivity.filter((document) => document.data().status === "rejected" && (timestampMs(document.data().updatedAt) ?? 0) > Date.now() - 30 * DAY_MS)
            .map((document) => timestampMs(document.data().suggestedStartAt)).filter((value) => value !== null));
        const slots = (0, engine_1.findAdaptiveTimeSlots)(request, 8).filter((slot) => Math.abs(slot.startMs - activity.startMs) >= 15 * MINUTE_MS && !rejectedStarts.has(slot.startMs));
        const selected = requestedStartMs ? slots.find((slot) => slot.startMs === requestedStartMs) : slots[0];
        if (!selected)
            throw new https_1.HttpsError("not-found", "ไม่พบช่วงว่างที่ผ่านเงื่อนไขทั้งหมดก่อนกำหนดส่ง");
        ...
        const addAlternative = (slot, label, tradeoff) => {
            if (!slot || selectedAlternatives.some((item) => item.slot.startMs === slot.startMs))
                return;
            selectedAlternatives.push({ label, slot, tradeoff });
        };
        addAlternative([...alternativeCandidates].sort((left, right) => left.startMs - right.startMs)[0], "ทางเลือกที่เริ่มได้เร็วที่สุด", "เริ่มได้เร็ว แต่คะแนนรวมอาจต่ำกว่าเวลาหลัก");
        addAlternative([...alternativeCandidates].sort((left, right) => left.breakdown.workloadPenalty - right.breakdown.workloadPenalty || right.totalScore - left.totalScore)[0], "วันที่ภาระเบากว่า", "ลดภาระรวมของวัน แต่อาจต้องเริ่มช้าหรือย้ายไปวันอื่น");
        addAlternative(alternativeCandidates.find((slot) => /ตรงกับ|สอดคล้องกับรูปแบบ/.test(slot.expectedBenefit)), "ตรงกับช่วงที่คุณถนัด", "อิงจากค่าที่ตั้งไว้หรือพฤติกรรมเดิม แต่ไม่ใช่เวลาที่เร็วที่สุด");
        ...
        const validUntilMs = Math.min(nowMs + PENDING_SUGGESTION_TTL_MS, Math.max(nowMs + 5 * MINUTE_MS, selected.startMs - 5 * MINUTE_MS));
        ...
        await reference.set({
            ...
            createdAt: firestore_1.FieldValue.serverTimestamp(),
            expectedBenefit: selected.expectedBenefit,
            expiresAt: firestore_1.Timestamp.fromMillis(validUntilMs),
            explanation,
            generatedForLocalDate: localDateKey(selected.startMs, setting.timeZone),
            generatedForTimeZone: setting.timeZone,
            mode: automatic ? "automatic" : "suggestion",
            observationCount: pattern?.observationCount ?? 0,
            originalEndAt: firestore_1.Timestamp.fromMillis(activity.endMs),
            originalScheduleVersion: activity.version,
            originalStartAt: firestore_1.Timestamp.fromMillis(activity.startMs),
            ownerId: uid,
            score: selected.totalScore,
            scoreBreakdown: selected.breakdown,
            scheduleItemId: activity.id,
            status: "pending",
            suggestedEndAt: firestore_1.Timestamp.fromMillis(selected.endMs),
            suggestedStartAt: firestore_1.Timestamp.fromMillis(selected.startMs),
            taskTitle: activity.title,
            updatedAt: firestore_1.FieldValue.serverTimestamp(),
    ...
    async function recordEvent(uid, data) {
        const eventType = text(data.eventType, 60);
        if (!types_1.ADAPTIVE_BEHAVIOR_EVENT_TYPES.includes(eventType)) {
            throw new https_1.HttpsError("invalid-argument", "eventType is invalid.");
        }
        const scheduleItemId = text(data.scheduleItemId, 128);
        if (!scheduleItemId)
            throw new https_1.HttpsError("invalid-argument", "scheduleItemId is required.");
        const [activity, setting] = await Promise.all([
            userRef(uid).collection("activities").doc(scheduleItemId).get(),
            preferences(uid),
        ]);
        if (!activity.exists)
            throw new https_1.HttpsError("not-found", "ไม่พบกิจกรรมที่ต้องการบันทึกพฤติกรรม");
        ...
        await reference.set({
            ...
            actualEnd: timestampMs(data.actualEnd) === null ? null : firestore_1.Timestamp.fromMillis(timestampMs(data.actualEnd)),
            actualStart: actualStartMs === null ? null : firestore_1.Timestamp.fromMillis(actualStartMs),
            createdAt: firestore_1.FieldValue.serverTimestamp(),
            ...
            timePeriod: (0, engine_1.adaptiveTimePeriod)(localHour(referenceMs, setting.timeZone)),
            updatedAt: firestore_1.FieldValue.serverTimestamp(),
            updatedScheduledStart: firestore_1.Timestamp.fromMillis(updatedStartMs),
        });
        return reference.id;
    }
    async function calculateUserPatterns(uid) {
        const setting = await preferences(uid);
        if (!setting.allowBehavioralPersonalization)
            return { patterns: 0 };
        const snapshot = await userRef(uid).collection("schedulingBehaviorEvents").orderBy("createdAt", "desc").limit(500).get();
        const observations = snapshot.docs.map((document) => {
            const data = document.data();
            return {
                ...
                category: category(data.activityCategory),
                eventType: text(data.eventType, 60),
                originalStartMs: timestampMs(data.originalScheduledStart),
                updatedStartMs: timestampMs(data.updatedScheduledStart),
            };
        }).filter((item) => types_1.ADAPTIVE_BEHAVIOR_EVENT_TYPES.includes(item.eventType));
        const patterns = (0, engine_1.calculateSchedulingPatterns)(observations, setting.thresholds, setting.timeZone);
    ...
    async function suggestionTransaction(uid, suggestionId, automatic) {
        ...
        const result = await db.runTransaction(async (transaction) => {
            ...
            if (!suggestionSnapshot.exists)
                throw new https_1.HttpsError("not-found", "ไม่พบคำแนะนำนี้");
            const suggestion = suggestionSnapshot.data() ?? {};
            if (suggestion.status !== "pending")
                throw new https_1.HttpsError("failed-precondition", "คำแนะนำนี้ถูกจัดการแล้ว");
            const expiresAt = timestampMs(suggestion.expiresAt);
            if (expiresAt !== null && expiresAt < Date.now())
                throw new https_1.HttpsError("failed-precondition", "คำแนะนำนี้หมดอายุแล้ว");
            const activityId = text(suggestion.scheduleItemId, 128);
            const activityReference = userRef(uid).collection("activities").doc(activityId);
            const activitySnapshot = await transaction.get(activityReference);
            if (!activitySnapshot.exists)
                throw new https_1.HttpsError("not-found", "ไม่พบกิจกรรมเดิม");
            ...
            if (activity.version !== finiteNumber(suggestion.originalScheduleVersion) || activity.startMs !== timestampMs(suggestion.originalStartAt)) {
                throw new https_1.HttpsError("aborted", "ตารางถูกแก้จากอุปกรณ์อื่น กรุณาสร้างคำแนะนำใหม่");
            }
            ...
            if (newStartMs === null || newEndMs === null)
                throw new https_1.HttpsError("data-loss", "เวลาที่แนะนำไม่สมบูรณ์");
            if (newStartMs < Date.now() + 5 * MINUTE_MS) {
                throw new https_1.HttpsError("failed-precondition", "เวลาที่แนะนำผ่านไปหรือใกล้เกินไปแล้ว กรุณาสร้างคำแนะนำใหม่");
            }
            ...
            if (automatic && (!setting.allowAutomaticRescheduling || finiteNumber(suggestion.confidence) < setting.minimumAutomaticConfidence)) {
                throw new https_1.HttpsError("failed-precondition", "ยังไม่ผ่านเกณฑ์การเลื่อนอัตโนมัติ");
            }
            const dayStart = newStartMs - DAY_MS;
            const dayEnd = newEndMs + DAY_MS;
            const schedulesQuery = userRef(uid).collection("schedules")
                .where("startAt", ">=", firestore_1.Timestamp.fromMillis(dayStart)).where("startAt", "<", firestore_1.Timestamp.fromMillis(dayEnd)).limit(300);
            ...
            scheduleSnapshot.docs.forEach((document) => {
                ...
                const endMs = timestampMs(data.endAt);
                if (startMs !== null && endMs !== null)
                    scheduleItems.push({ category: "study", endMs, id: document.id, isDifficult: true, isFixed: true, startMs });
            });
            activityConstraintsSnapshot.docs.forEach((document) => {
                if (document.id === activity.id)
                    return;
                const item = activityFromDocument(document);
                if (["cancelled", "completed"].includes(item.status))
                    return;
            ...
            const validation = (0, engine_1.validateCandidateSlot)({
                category: activity.category,
                deadlineMs: activity.deadlineMs,
            ...
            if (!validation.ok)
                throw new https_1.HttpsError("failed-precondition", validation.message ?? "ช่วงเวลานี้ไม่ผ่านการตรวจสอบ");
            ...
            transaction.set(historyReference, {
                ...
                automatic,
                canUndoUntil: firestore_1.Timestamp.fromMillis(Date.now() + PENDING_SUGGESTION_TTL_MS),
                createdAt: firestore_1.FieldValue.serverTimestamp(),
                newEndAt: firestore_1.Timestamp.fromMillis(newEndMs),
                newScheduleVersion: nextVersion,
                newStartAt: firestore_1.Timestamp.fromMillis(newStartMs),
                ownerId: uid,
                previousEndAt: firestore_1.Timestamp.fromMillis(activity.endMs),
                previousScheduleVersion: activity.version,
                previousStartAt: firestore_1.Timestamp.fromMillis(activity.startMs),
            ...
            transaction.update(activityReference, {
                ...
                originalScheduledStart: activitySnapshot.data()?.originalScheduledStart ?? firestore_1.Timestamp.fromMillis(activity.startMs),
                scheduleVersion: nextVersion,
                startAt: firestore_1.Timestamp.fromMillis(newStartMs),
                endAt: firestore_1.Timestamp.fromMillis(newEndMs),
                updatedAt: firestore_1.FieldValue.serverTimestamp(),
            });
            transaction.update(suggestionReference, { acceptedAt: firestore_1.FieldValue.serverTimestamp(), status: "accepted", updatedAt: firestore_1.FieldValue.serverTimestamp() });
            transaction.set(eventReference, {
                ...
                originalScheduledStart: firestore_1.Timestamp.fromMillis(activity.startMs),
                ownerId: uid,
                scheduleItemId: activity.id,
                source: automatic ? "automatic_scheduler" : "ai_suggestion",
                timePeriod: (0, engine_1.adaptiveTimePeriod)(localHour(newStartMs, setting.timeZone)),
                updatedAt: firestore_1.FieldValue.serverTimestamp(),
                updatedScheduledStart: firestore_1.Timestamp.fromMillis(newStartMs),
            });
            transaction.set(notificationReference, {
                createdAt: firestore_1.FieldValue.serverTimestamp(), kind: "schedule", message: `ย้าย ${activity.title} ไปยังเวลาที่ผ่านการตรวจสอบแล้ว`,
                ownerId: uid, read: false, title: automatic ? "ปรับตารางอัตโนมัติแล้ว" : "ปรับตารางแล้ว", updatedAt: firestore_1.FieldValue.serverTimestamp(),
            });
    ...
    async function rejectSuggestion(uid, suggestionId) {
        ...
        await db.runTransaction(async (transaction) => {
            ...
            const eventReference = userRef(uid).collection("schedulingBehaviorEvents").doc();
            transaction.update(reference, { rejectedAt: firestore_1.FieldValue.serverTimestamp(), status: "rejected", updatedAt: firestore_1.FieldValue.serverTimestamp() });
            transaction.set(eventReference, {
                activityCategory: category(data.activityCategory), actualDurationMinutes: null, actualEnd: null, actualStart: null,
                createdAt: firestore_1.FieldValue.serverTimestamp(), dayOfWeek: dayOfWeek(timestampMs(data.suggestedStartAt) ?? Date.now(), setting.timeZone),
                ...
                ownerId: uid, scheduleItemId: activityId, source: "ai_suggestion", timePeriod: (0, engine_1.adaptiveTimePeriod)(localHour(timestampMs(data.suggestedStartAt) ?? Date.now(), setting.timeZone)),
                updatedAt: firestore_1.FieldValue.serverTimestamp(), updatedScheduledStart: data.suggestedStartAt ?? null,
            });
        });
        return { ok: true };
    }
    async function alternativeTime(uid, suggestionId, startMs) {
        const reference = userRef(uid).collection("schedulingSuggestions").doc(suggestionId);
        const suggestion = await reference.get();
        if (!suggestion.exists || suggestion.data()?.status !== "pending")
            throw new https_1.HttpsError("failed-precondition", "คำแนะนำนี้ไม่พร้อมแก้ไข");
        ...
        if (!validation.ok)
            throw new https_1.HttpsError("failed-precondition", validation.message ?? "ช่วงเวลานี้ใช้ไม่ได้");
        ...
        await db.runTransaction(async (transaction) => {
            ...
            if (!freshSuggestion.exists || freshSuggestionData.status !== "pending") {
                throw new https_1.HttpsError("failed-precondition", "คำแนะนำนี้ถูกยืนยัน ปฏิเสธ หรือหมดอายุแล้ว");
            }
            ...
            if (freshExpiresAtMs === null || freshValidUntilMs === null ||
                ...
                throw new https_1.HttpsError("failed-precondition", "คำแนะนำนี้หมดอายุแล้ว กรุณาสร้างคำแนะนำใหม่");
            }
            const freshActivity = await transaction.get(userRef(uid).collection("activities").doc(activityId));
            if (!freshActivity.exists)
                throw new https_1.HttpsError("not-found", "ไม่พบกิจกรรมเดิม");
            ...
            if (finiteNumber(freshActivityData.scheduleVersion) !== finiteNumber(freshSuggestionData.originalScheduleVersion) ||
                ...
                throw new https_1.HttpsError("aborted", "ตารางเปลี่ยนแล้ว กรุณาสร้างคำแนะนำใหม่ก่อนเลือกเวลาอื่น");
            }
            if (startMs < Date.now() + SUGGESTION_MINIMUM_LEAD_MS) {
                throw new https_1.HttpsError("failed-precondition", "เวลาที่เลือกใกล้หรือผ่านไปแล้ว กรุณาเลือกเวลาใหม่");
            }
            transaction.update(reference, {
                expectedBenefit: "ช่วงเวลาที่ผู้ใช้เลือกและผ่านการตรวจสอบ deterministic",
                expiresAt: firestore_1.Timestamp.fromMillis(validUntilMs),
                suggestedEndAt: firestore_1.Timestamp.fromMillis(endMs),
                suggestedStartAt: firestore_1.Timestamp.fromMillis(startMs),
                updatedAt: firestore_1.FieldValue.serverTimestamp(),
                userModified: true,
                validUntil: firestore_1.Timestamp.fromMillis(validUntilMs),
            });
    ...
    async function dashboard(uid) {
        ...
        const weekEnd = firestore_1.Timestamp.fromMillis(Date.now() + 7 * DAY_MS);
    ...
    async function createFastSuggestions(uid, items, limit = 3) {
        const suggestions = [];
        let attempted = 0;
    ...
    async function createDayRebalanceSuggestions(uid, targetMs) {
        const setting = await preferences(uid);
        const dayStart = (0, engine_1.zonedDayStart)(targetMs, setting.timeZone);
        const dayEnd = (0, engine_1.zonedDayStart)(dayStart, setting.timeZone, 1);
        const snapshot = await userRef(uid).collection("activities")
            .where("startAt", ">=", firestore_1.Timestamp.fromMillis(dayStart))
            .where("startAt", "<", firestore_1.Timestamp.fromMillis(dayEnd)).limit(100).get();
    ...
    async function createWeekRebalanceSuggestions(uid, targetMs) {
        const setting = await preferences(uid);
        const weekStart = (0, engine_1.zonedDayStart)(targetMs, setting.timeZone);
        const weekEnd = (0, engine_1.zonedDayStart)(weekStart, setting.timeZone, 7);
        const [snapshot, scheduleItems] = await Promise.all([
            userRef(uid).collection("activities")
                .where("startAt", ">=", firestore_1.Timestamp.fromMillis(weekStart)).where("startAt", "<", firestore_1.Timestamp.fromMillis(weekEnd)).limit(200).get(),
            constraints(uid, weekStart, weekEnd),
        ]);
        const workloadByDay = new Map();
    ...
    async function activateAdaptiveForUser(uid) {
    ...
    const createAdaptiveActivity = (0, https_1.onCall)(callableOptions, async (request) => {
        ...
        const committedResult = (data) => {
            ...
            if (data.ownerId !== uid || (clientRequestId && text(data.clientRequestId, 256) !== clientRequestId) || existingStartMs === null || existingEndMs === null) {
                throw new https_1.HttpsError("data-loss", "พบข้อมูลคำขอสร้างกิจกรรมเดิมที่ไม่สอดคล้องกัน");
            }
            return {
                adjusted: false,
                conflicts: [],
        ...
        return db.runTransaction(async (transaction) => {
            ...
            suggestionSnapshot.docs.forEach((document) => {
                ...
                const expiresAt = timestampMs(data.expiresAt) ?? timestampMs(data.validUntil) ?? 0;
                if (itemStartMs === null || itemEndMs === null || itemEndMs <= startMs || itemStartMs >= endMs || expiresAt <= Date.now())
                    return;
                freshScheduleItems.push({
            ...
            transaction.create(eventReference, {
                ...
                source: "ai_suggestion",
                timePeriod: (0, engine_1.adaptiveTimePeriod)(localHour(startMs, freshSetting.timeZone)),
                updatedAt: firestore_1.FieldValue.serverTimestamp(),
                updatedScheduledStart: firestore_1.Timestamp.fromMillis(startMs),
            });
            return {
    ...
    const getAdaptiveSchedulingDashboard = (0, https_1.onCall)(callableOptions, async (request) => dashboard(requiredUid(request)));
    const updateAdaptiveSchedulingPreferences = (0, https_1.onCall)(callableOptions, async (request) => {
        const uid = requiredUid(request);
    ...
    const generateAdaptiveSuggestion = (0, https_1.onCall)({ ...callableOptions, secrets: [geminiApiKey] }, async (request) => {
        ...
        if (!activityId)
            throw new https_1.HttpsError("invalid-argument", "activityId is required.");
        return createSuggestion(uid, activityId);
    });
    const acceptSchedulingSuggestion = (0, https_1.onCall)(callableOptions, async (request) => {
        const uid = requiredUid(request);
        const suggestionId = text(request.data?.suggestionId, 128);
        if (!suggestionId)
            throw new https_1.HttpsError("invalid-argument", "suggestionId is required.");
        return suggestionTransaction(uid, suggestionId, false);
    });
    const rejectSchedulingSuggestion = (0, https_1.onCall)(callableOptions, async (request) => {
        const uid = requiredUid(request);
        const suggestionId = text(request.data?.suggestionId, 128);
        if (!suggestionId)
            throw new https_1.HttpsError("invalid-argument", "suggestionId is required.");
    ...
    const chooseAlternativeSchedulingTime = (0, https_1.onCall)(callableOptions, async (request) => {
        const uid = requiredUid(request);
        const suggestionId = text(request.data?.suggestionId, 128);
        const startMs = timestampMs(request.data?.startAt);
        if (!suggestionId || startMs === null)
            throw new https_1.HttpsError("invalid-argument", "suggestionId and startAt are required.");
        return alternativeTime(uid, suggestionId, startMs);
    });
    const lockAdaptiveScheduleItem = (0, https_1.onCall)(callableOptions, async (request) => {
        const uid = requiredUid(request);
        const activityId = text(request.data?.activityId, 128);
        if (!activityId)
            throw new https_1.HttpsError("invalid-argument", "activityId is required.");
        ...
        await db.runTransaction(async (transaction) => {
            const snapshot = await transaction.get(reference);
            if (!snapshot.exists)
                throw new https_1.HttpsError("not-found", "ไม่พบกิจกรรมนี้");
    ...
    const undoScheduleChange = (0, https_1.onCall)(callableOptions, async (request) => {
        const uid = requiredUid(request);
        const historyId = text(request.data?.historyId, 128);
        if (!historyId)
            throw new https_1.HttpsError("invalid-argument", "historyId is required.");
        ...
        const result = await db.runTransaction(async (transaction) => {
            ...
            if (!historySnapshot.exists)
                throw new https_1.HttpsError("not-found", "ไม่พบประวัติการเปลี่ยนแปลง");
            const history = historySnapshot.data() ?? {};
            if (history.status !== "applied")
                throw new https_1.HttpsError("failed-precondition", "รายการนี้ย้อนกลับแล้วหรือใช้ไม่ได้");
            if ((timestampMs(history.canUndoUntil) ?? 0) < Date.now())
                throw new https_1.HttpsError("failed-precondition", "หมดเวลาสำหรับการย้อนกลับแล้ว");
            ...
            if (activity.version !== finiteNumber(history.newScheduleVersion) || activity.startMs !== timestampMs(history.newStartAt)) {
                throw new https_1.HttpsError("aborted", "กิจกรรมถูกแก้จากอุปกรณ์อื่น จึงย้อนกลับอัตโนมัติไม่ได้");
            }
            ...
            if (previousStartMs === null || previousEndMs === null)
                throw new https_1.HttpsError("data-loss", "ประวัติเวลาเดิมไม่สมบูรณ์");
            if (previousStartMs < Date.now() + 5 * MINUTE_MS) {
                throw new https_1.HttpsError("failed-precondition", "เวลาเดิมผ่านไปหรือใกล้เกินไปแล้ว จึงไม่สามารถย้อนกลับได้");
            }
            const settingSnapshot = await transaction.get(settingsRef(uid));
            const setting = sanitizePreferences(settingSnapshot.data() ?? {});
            const rangeStart = firestore_1.Timestamp.fromMillis(previousStartMs - DAY_MS);
            const rangeEnd = firestore_1.Timestamp.fromMillis(previousEndMs + DAY_MS);
            const scheduleSnapshot = await transaction.get(userRef(uid).collection("schedules")
            ...
            const restoreValidation = (0, engine_1.validateCandidateSlot)({
                category: activity.category,
                deadlineMs: activity.deadlineMs,
            ...
            if (!restoreValidation.ok) {
                throw new https_1.HttpsError("failed-precondition", `คืนเวลาเดิมไม่ได้: ${restoreValidation.message ?? "ช่วงเวลาเดิมไม่ว่างแล้ว"}`);
            }
            transaction.update(activityReference, {
                aiReason: null,
                aiScheduled: false,
                endAt: firestore_1.Timestamp.fromMillis(previousEndMs),
                scheduleVersion: activity.version + 1,
                startAt: firestore_1.Timestamp.fromMillis(previousStartMs),
                updatedAt: firestore_1.FieldValue.serverTimestamp(),
            });
            transaction.update(historyReference, { status: "undone", undoneAt: firestore_1.FieldValue.serverTimestamp(), updatedAt: firestore_1.FieldValue.serverTimestamp() });
            transaction.set(userRef(uid).collection("schedulingBehaviorEvents").doc(), {
                ...
                createdAt: firestore_1.FieldValue.serverTimestamp(), dayOfWeek: dayOfWeek(previousStartMs, setting.timeZone), estimatedDurationMinutes: activity.estimatedDurationMinutes,
                eventType: "automatic_change_undone", metadata: { historyId }, originalScheduledStart: history.newStartAt,
                ownerId: uid, scheduleItemId: activity.id, source: "user", timePeriod: (0, engine_1.adaptiveTimePeriod)(localHour(previousStartMs, text(history.timeZone, 80) || "Asia/Bangkok")),
                updatedAt: firestore_1.FieldValue.serverTimestamp(), updatedScheduledStart: history.previousStartAt,
            });
    ...
    const deleteSchedulingPattern = (0, https_1.onCall)(callableOptions, async (request) => {
        const uid = requiredUid(request);
        const patternId = text(request.data?.patternId, 128);
        if (!patternId)
            throw new https_1.HttpsError("invalid-argument", "patternId is required.");
        await userRef(uid).collection("schedulingPatterns").doc(patternId).delete();
        return { ok: true };
    });
    const deleteSchedulingBehaviorHistory = (0, https_1.onCall)(callableOptions, async (request) => {
        const uid = requiredUid(request);
        let deleted = 0;
        while (deleted < 5000) {
    ...
    const registerAdaptivePushToken = (0, https_1.onCall)(callableOptions, async (request) => {
        const uid = requiredUid(request);
        ...
        if (!token || !["android", "ios"].includes(platform))
            throw new https_1.HttpsError("invalid-argument", "A native Android or iOS push token is required.");
    ...
    const activateAdaptiveScheduling = (0, https_1.onCall)(callableOptions, async (request) => {
        return activateAdaptiveForUser(requiredUid(request));
    });
    const processNaturalLanguageScheduleCommand = (0, https_1.onCall)({ ...callableOptions, secrets: [geminiApiKey] }, async (request) => {
        const uid = requiredUid(request);
        ...
        if (intent.intent === "find_time") {
            ...
            if (!matched)
                return { ...response, ...await proposeNewFlexibleActivity(uid, intent, message) };
            response.suggestion = await createSuggestion(uid, matched.id, false, undefined, true, requestedWindowForIntent(intent, matched.estimatedDurationMinutes || matched.durationMinutes), intent.requestedLocalDate ?? undefined);
            return response;
        }
        if (intent.intent === "rebalance_day") {
            response.suggestions = await createDayRebalanceSuggestions(uid, (0, engine_1.zonedDayStart)(Date.now(), setting.timeZone, 1));
            return response;
        }
        if (intent.intent === "rebalance_week") {
            response.suggestions = await createWeekRebalanceSuggestions(uid, (0, engine_1.zonedDayStart)(Date.now(), setting.timeZone));
            return response;
        }
    ...
    const rebalanceUserDay = (0, https_1.onCall)({ ...callableOptions, secrets: [geminiApiKey] }, async (request) => {
        const uid = requiredUid(request);
        const setting = await preferences(uid);
        const targetMs = timestampMs(request.data?.date) ?? (0, engine_1.zonedDayStart)(Date.now(), setting.timeZone, 1);
        return { suggestions: await createDayRebalanceSuggestions(uid, targetMs) };
    });
    const rebalanceUserWeek = (0, https_1.onCall)({ ...callableOptions, secrets: [geminiApiKey] }, async (request) => {
        const uid = requiredUid(request);
        const setting = await preferences(uid);
    ...
    const scheduledAdaptivePatternRecalculation = (0, scheduler_1.onSchedule)({ region, schedule: "every day 03:15", timeZone: "Asia/Bangkok" }, async () => {
        const recent = await db.collectionGroup("schedulingBehaviorEvents")
            .where("createdAt", ">=", firestore_1.Timestamp.fromMillis(Date.now() - 2 * DAY_MS)).limit(1000).get();
        ...
        for (const uid of users.slice(0, 200)) {
            try {
                await calculateUserPatterns(uid);
            }
            catch (error) {
                console.error("Adaptive pattern recalculation failed.", { error, uid });
            }
        }
    });
    const scheduledAutomaticAdaptiveScheduling = (0, scheduler_1.onSchedule)({ region, schedule: "every day 04:15", secrets: [geminiApiKey], timeZone: "Asia/Bangkok" }, async () => {
        ...
        for (const settingDocument of settings.docs.filter((document) => document.id === "adaptiveScheduling")) {
            ...
            const tomorrow = (0, engine_1.zonedDayStart)(Date.now(), setting.timeZone, 1);
            const tomorrowEnd = (0, engine_1.zonedDayStart)(tomorrow, setting.timeZone, 1);
            const activities = await userRef(uid).collection("activities")
                .where("startAt", ">=", firestore_1.Timestamp.fromMillis(tomorrow)).where("startAt", "<", firestore_1.Timestamp.fromMillis(tomorrowEnd)).limit(50).get();
            for (const activity of activities.docs.map(activityFromDocument).filter((item) => item.isFlexible && !item.isLocked && item.allowAiReschedule).slice(0, 2)) {
                try {
                    ...
                    if (suggestion.confidence >= setting.minimumAutomaticConfidence)
                        await suggestionTransaction(uid, suggestion.id, true);
                }
                catch (error) {
