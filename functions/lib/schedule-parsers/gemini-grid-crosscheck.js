...
exports.GRID_FIELD_LABELS = exports.GRID_EXTRACTION_PROMPT = void 0;
exports.extractScheduleGridWithGemini = extractScheduleGridWithGemini;
exports.normalizeGridDay = normalizeGridDay;
exports.ocrEvidence = ocrEvidence;
exports.textInOcr = textInOcr;
exports.inOcr = inOcr;
exports.crossCheckScheduleEntries = crossCheckScheduleEntries;
...
exports.GRID_EXTRACTION_PROMPT = `You read a weekly class timetable image for a Thai student app.
...
async function extractScheduleGridWithGemini({ apiKey, imageDataUrl, ocrText, timeoutMs = 18000, }) {
        ...
        try {
            const response = await fetch("https://generativelanguage.googleapis.com/v1/interactions", {
                ...
                body: JSON.stringify({
                    model,
                    store: false,
                    system_instruction: exports.GRID_EXTRACTION_PROMPT,
            ...
            if (timedOut && index < MODELS.length - 1)
                continue;
            return null;
        }
        finally {
...
const squash = (value) => text(value).toUpperCase().replace(/[^A-Z0-9ก-๙]/g, "");
/**
...
function ocrEvidence(ocrText, gridDays = []) {
    ...
    const days = gridDays.length ?
        new Set(gridDays) :
        new Set([...tokens].map(normalizeGridDay).filter((day) => Boolean(day)));
...
function textInOcr(value, evidence) {
    const squashed = squash(value);
    return squashed.length >= 3 && evidence.squashed.includes(squashed);
}
function inOcr(field, value, evidence) {
    if (field === "day")
        return evidence.days.has(normalizeGridDay(value) ?? value);
    if (field === "startTime" || field === "endTime")
        return evidence.times.has(value);
    const squashed = squash(value);
    if (!squashed)
        return false;
    if (field === "courseCode")
        return squashed.length >= 4 && evidence.squashed.includes(squashed);
...
function same(field, a, b) {
    if (field === "section")
        return squash(a).replace(/^0+(?=\d)/, "") === squash(b).replace(/^0+(?=\d)/, "");
    if (field === "day" || field === "startTime" || field === "endTime")
        return a === b;
    return squash(a) === squash(b);
}
function modelValue(field, course) {
    if (field === "day")
        return normalizeGridDay(course.day);
    if (field === "startTime")
        return normalizeTime(course.start_time);
    if (field === "endTime")
        return normalizeTime(course.end_time);
    if (field === "room")
        return unlabelled("room", text(course.room)) || null;
    if (field === "section")
        return unlabelled("section", text(course.section)) || null;
    return text(course.course_code).toUpperCase() || null;
}
function flag(entry, field, note) {
    entry.reviewFields = [...new Set([...(entry.reviewFields ?? []), field])];
    entry.reviewNotes = [...(entry.reviewNotes ?? []), note];
}
function crossCheckScheduleEntries(base, courses, ocrText, gridDays = []) {
    const evidence = ocrEvidence(ocrText, gridDays);
    const stats = { added: 0, agreed: 0, conflicts: 0, dropped: 0, filled: 0, flagged: 0, geminiCourses: courses.length, rejected: 0 };
    const entries = base.map((entry) => ({ ...entry, reviewFields: [...(entry.reviewFields ?? [])], reviewNotes: [...(entry.reviewNotes ?? [])] }));
    const used = new Set();
    const merge = (entry, course) => {
        for (const field of ["section", "room", "day", "startTime", "endTime"]) {
            ...
            if (current) {
                if (same(field, current, offered))
                    stats.agreed += 1;
                else if (verified) {
                    // Both readings are real words on the page -- a genuine ambiguity.
                    flag(entry, field, `${exports.GRID_FIELD_LABELS[field]}: ระบบอ่านได้ "${current}" แต่ AI อ่านเป็น "${offered}"`);
                    stats.conflicts += 1;
                }
                else
                    stats.rejected += 1;
            ...
            if (verified) {
                ...
                flag(entry, field, `${exports.GRID_FIELD_LABELS[field]}: AI อ่านได้ "${offered}" แต่ไม่พบในข้อความที่สแกน จึงยังไม่กรอกให้`);
                stats.flagged += 1;
            }
        }
        if (entry.startTime && entry.endTime) {
            if (entry.startTime >= entry.endTime)
                flag(entry, "endTime", "เวลาสิ้นสุดต้องอยู่หลังเวลาเริ่ม");
            else
    ...
    for (const course of courses) {
        ...
        if (index < 0)
            index = entries.findIndex((entry, at) => !used.has(at) && squash(entry.courseCode) === key);
        if (index >= 0) {
            used.add(index);
...
//# sourceMappingURL=gemini-grid-crosscheck.js.map