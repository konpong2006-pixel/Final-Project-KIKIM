...
exports.STRUCTURED_MODELS = void 0;
exports.requestStructuredJson = requestStructuredJson;
/** Newest first; an older model answers when a newer one is missing or busy. */
exports.STRUCTURED_MODELS = ["gemini-3.8-flash", "gemini-3.7-flash", "gemini-3.6-flash", "gemini-3.5-flash"];
/**
 ...
 */
async function requestStructuredJson({ apiKey, imageDataUrl, label, prompt, schema, text, timeoutMs = 18000, }) {
    const image = imageDataUrl.match(/^data:(image\/[a-z0-9.+-]+);base64,([\s\S]+)$/i);
    if (!apiKey || !image)
        return null;
    for (const [index, model] of exports.STRUCTURED_MODELS.entries()) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (retryable && index < exports.STRUCTURED_MODELS.length - 1)
                    continue;
            ...
            const answer = payload.steps
                ...
                .join("")
                .trim() ?? "";
            return { model, value: JSON.parse(answer) };
        }
        catch (error) {
            ...
            if (timedOut && index < exports.STRUCTURED_MODELS.length - 1)
                continue;
            return null;
        }
        finally {
...
//# sourceMappingURL=gemini-structured.js.map