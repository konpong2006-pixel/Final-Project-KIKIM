...
exports.parseListSchedule = parseListSchedule;
...
function room(context) {
    ...
    const usable = candidates.filter((candidate) => !candidate.includes(":") && !COURSE_CODE_ANCHORED.test(candidate) && !/^\d{4}$/.test(candidate));
...
function courseName(firstLine, context, rawCode) {
...
function parseListSchedule(rawText) {
    ...
    const entries = starts.flatMap((lineIndex, position) => {
...
//# sourceMappingURL=list-parser.js.map