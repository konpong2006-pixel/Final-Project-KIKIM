...
type GeminiTemporalPayload = {
  academic_year?: unknown;
  calendar_confidence?: unknown;
  calendar_evidence?: unknown;
  entries?: unknown;
...
export type ScheduleTemporalReviewResult = {
...
function temporalCandidates(entries: StandardScheduleEntry[]) {
...
export async function reviewScheduleTemporalFieldsWithGemini({
  apiKey,
  entries,
  imageDataUrl,
  rawText,
  ...
  if (!entries.length) {
    return {
      ...
      semesterEnd: null,
      semesterStart: null,
    } satisfies ScheduleTemporalReviewResult;
  }
  ...
  if (!outputText) throw new Error("Gemini returned no schedule temporal output.");
  return normalizePayload(JSON.parse(outputText) as unknown, entries);
}
