  ...
  for (let y = radius; y < h - radius; y += 1) {
    for (let x = radius; x < w - radius; x += 1) {
      ...
      for (let dy = -radius; dy <= radius && keep; dy += 1) {
        const row = (y + dy) * w;
        if (!mask[row + x - radius] || !mask[row + x + radius] || !mask[row + x]) keep = 0;
      }
      for (let dx = -radius; dx <= radius && keep; dx += 1) {
        if (!mask[y * w + x + dx]) keep = 0;
      }
      eroded[y * w + x] = keep;
    }
  }
...
export function ruledLines(
  ...
  const medianOf = (line: number) => {
    if (line < 0 || line >= along) return null;
    const channels: number[][] = [[], [], []];
  ...
  const at = (line: number) => {
    if (!medians.has(line)) medians.set(line, medianOf(line));
    return medians.get(line) ?? null;
  };
  const apart = (a: number[] | null, b: number[] | null) =>
    !a || !b ? 0 : Math.abs(a[0] - b[0]) + Math.abs(a[1] - b[1]) + Math.abs(a[2] - b[2]);
  const k = 3;
  const hits: number[] = [];
  for (let line = rangeFrom; line <= rangeTo; line += 1) {
    const here = at(line);
    if (apart(here, at(line - k)) >= 18 && apart(here, at(line + k)) >= 18) hits.push(line);
  }
  const lines: number[] = [];
  let run: number[] = [];
