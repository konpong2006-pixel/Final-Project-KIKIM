...
/**
 * Reads a weekly timetable grid -- days down the side, times across the top --
 * from Cloud Vision's word geometry, one grid cell at a time.
 *
 * This replaces reading the timetable as text. The schedule OCR text is two
 * transcripts of the same image joined together: iApp reads a grid row by row,
 * Google Vision reads it column by column. Parsing that string saw every
 * course twice, in two different orders, and paired them with days by
 * reading order, so a seven-course timetable came back as thirteen entries
 * and a malformed twin like "DMR30364" could inherit another course's room.
 * Coordinates say which words share a cell; reading order cannot.
 *
 * Only the Vision annotation is read here, so nothing is counted twice.
 */
...
export type ScheduleGridResult = {
  ...
  found: boolean;
  /** Day headers actually read, top to bottom -- used to validate a model's days. */
  rows: GridRow[];
  skewDegrees: number;
};

export const THAI_DAYS = ["จันทร์", "อังคาร", "พุธ", "พฤหัสบดี", "ศุกร์", "เสาร์", "อาทิตย์"] as const;
...
function skewSlope(words: Word[]) {
  ...
  for (let i = 0; i < times.length; i += 1) {
    for (let j = i + 1; j < times.length; j += 1) {
      ...
      if (Math.abs(dx) < 3 * Math.max(times[i].height, times[j].height)) continue;
      const slope = (times[j].cy - times[i].cy) / dx;
      if (Math.abs(slope) < 0.27) slopes.push(slope);
    }
  }
...
export function joinLine(line: Word[]) {
  ...
  for (const word of line) {
    if (previous) {
      ...
      if (!tight && word.left - previous.right > Math.min(previous.height, word.height) * 0.35) out += " ";
    }
    out += word.text;
    previous = word;
  }
...
function headerColumns(words: Word[]) {
  ...
  if (!best) return null;
  const columns: {minutes: number; word: Word}[] = [];
  let last = -1;
  ...
  return {
    ...
    columns: columns.map((column, index): GridColumn => ({
...
function dayRows(words: Word[], gridLeft: number, headerBottom: number, cutoff: number) {
  const column = words.filter((word) => word.cx < gridLeft && word.cy > headerBottom && word.cy < cutoff);
  const rows: {day: number; y: number}[] = [];
  for (const line of groupLines(column)) {
    ...
    if (day !== undefined) rows.push({day, y: median(line.map((word) => word.cy))});
  }
 ...
 */
function rowPitch(rows: {day: number; y: number}[]) {
  const steps: number[] = [];
 ...
 * guessed.
 */
function dayFor(y: number, rows: {day: number; y: number}[], pitch: number, height: number) {
  if (!rows.length) return null;
  const nearest = rows.reduce((best, row) => (Math.abs(row.y - y) < Math.abs(best.y - y) ? row : best));
  if (!pitch) return Math.abs(nearest.y - y) <= height * 3 ? nearest.day : null;
  const offset = (y - nearest.y) / pitch;
  const steps = Math.round(offset);
  if (Math.abs(offset - steps) > 0.45) return null;
 ...
 */
function readCell(lines: string[]) {
  let section: string | null = null;
  let room: string | null = null;
  let time: {end: string; start: string} | null = null;
  ...
  for (const line of rest) {
    ...
    if (!name && /[A-Za-zก-๙]{3,}/.test(token)) name = token;
  }
  return {name, room, section, time};
}

export function parseScheduleGrid(annotation: unknown): ScheduleGridResult {
  const nothing: ScheduleGridResult = {columns: [], entries: [], found: false, rows: [], skewDegrees: 0};
  const raw = rawWords(annotation);
  if (raw.length < 6) return nothing;
  ...
  if (!header) return {...nothing, skewDegrees};
  const height = median(words.map((word) => word.height)) || 20;
  ...
  const gridWords = words.filter((word) =>
    !headerWords.has(word) && word.cy > header.bottom && word.cy < cutoff &&
  ...
  for (const code of codes) {
    ...
    const yEnd = Math.min(
      code.word.top + rowHeight * 0.95,
      underneath.length ? Math.min(...underneath.map((other) => other.word.top)) - 1 : Number.POSITIVE_INFINITY,
      gridBottom,
    );
  ...
  return {
    ...
    rows: rows.map((row) => ({day: THAI_DAYS[row.day], y: row.y})),
    skewDegrees,
  };
}
