...
function TabBar({active, go}: {active: string; go: (page: string) => void}) {
  ...
  return <View style={styles.tabs}>{tabs.map(([icon, name, target], index) => <Touchable key={target} onPress={() => go(target)} style={styles.tab}>{index === 2 ? <View style={styles.plus}><Text style={styles.plusText}>{icon}</Text></View> : <><Text style={[styles.tabIcon, active === target && styles.active]}>{icon}</Text><Text style={[styles.tabText, active === target && styles.active]}>{name}</Text></>}</Touchable>)}</View>;
}
...
export default function UserPortal({onNavigate, page, uid}: Props) {
  ...
  const body = useMemo(() => {
    ...
    if (page === 'index') return <>
      ...
      <Card><Text style={styles.cardTitle}>AI Assistant</Text><Text style={styles.aiPrompt}>“วันนี้ฉันมีเรียนกี่โมง?”</Text><Touchable onPress={() => onNavigate('smartlife_ai_assistant')}><Text style={styles.link}>เปิด AI Assistant ›</Text></Touchable></Card>
    ...
    if (page.includes('scan') || page.includes('receipt')) return <Card><Text style={styles.cardTitle}>Smart Scan</Text><Text style={styles.caption}>สแกนตารางเรียนหรือใบเสร็จผ่านกล้องและคลังรูปภาพ</Text><Touchable onPress={() => onNavigate('smartlife_scan_schedule')} style={styles.primary}><Text style={styles.primaryText}>เปิดกล้องสแกน</Text></Touchable></Card>;
...
function ModeTabs({active, go, modes}: {active: string; go: (page: string) => void; modes: [string, string][]}) { return <View style={styles.mode}>{modes.map(([labelText, target]) => <Touchable key={target} onPress={() => go(target)} style={[styles.modeItem, active === target && styles.modeActive]}><Text style={[styles.modeText, active === target && styles.modeTextActive]}>{labelText}</Text></Touchable>)}</View>; }
